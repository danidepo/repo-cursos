...
#Generamos series temporales con ts
s.ts <- ts(s)
class(s.ts)
head(s.ts)
plot(s.ts)

#Generamos series temporales, desde el año 2001
...
#Si queremos que pinte las dos series en el mismo grafico
...
# Se descomponen los valores en estacion, tendencia y ruido en relacion al primero (data)
