...
#Convertimos la variable region en binaria 
dms <- dummy(edu$region, sep = "_")
edu <- cbind(edu, dms)

#Antes de aplicar el modelo knn, convertimos los valores a 0,1 con la funcion scale
...
#Creamos el conjunto de entrenamiento con la variable expense
...
#Probamos sobre el conjunto de validacion
...
#Knn sin particion de testing (test = NULL)
...
rdacb.knn.reg.multi <- function(tr_predictors, val_predictors,
  ...
  for (k in start_k:end_k) {
    rms_error <- rdacb.knn.reg(tr_predictors, val_predictors, tr_target, val_target, k)
